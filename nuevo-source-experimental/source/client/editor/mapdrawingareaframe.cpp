...
bool MapDrawingAreaFrame::onMapDrawingAreaPressed(GdkEventButton* event) {
    ...
    if (this->editor != NULL && this->mapDrawingArea.isImageLoaded()) {
	    if (event->x <= this->mapDrawingArea.getWidth() &&
            event->y <= this->mapDrawingArea.getHeight()) {
	        // Creo un label para pedirle el nombre del pais alusuario.
